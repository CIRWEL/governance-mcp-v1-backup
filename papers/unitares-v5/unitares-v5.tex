...
and verify $F + 2\alpha_c M \prec 0$ by direct eigenvalue computation.
The diagonal entries $F_{ii} = 2m_i J_{ii}$ are negative (from
$\alpha$, $\gamma_I$, $\mu$, $\delta$), and the metric weights $m_i$
are chosen to control the off-diagonal coupling from $\kappa$,
$\beta_I C'_{\max}$, and $k$. The full computation with numerical
verification for production parameters is given in
...
$\{0.42, 0.25, 0.8, 0.4\}$ and the coupling is dominated by
$\kappa = 0.3$ and $\beta_I C'_{\max} = 0.15$. The contraction
analysis (\Cref{app:contraction-proof}) with diagonal metric
$M = \operatorname{diag}(1.0, 2.5, 5.0, 2.5)$ certifies
$\alpha_c = 0.14$. The spectral abscissa of the Jacobian $J$ at
equilibrium is $-0.15$, confirming that the certified rate is
tight.
\end{remark}
...
from unpredictable task inputs. We model this as an It\^{o} \cite{ito1944} SDE:
\begin{equation}
...
\]
Setting $m_S = 5.0$ (from the contraction metric \eqref{eq:metric}),
$\sigma_{\max}^2 = (0.02)^2 = 4 \times 10^{-4}$. With the certified
contraction rate $\alpha_c = 0.14$ (\Cref{app:contraction-proof}),
the steady-state variance bound from \eqref{eq:mean-square} is:
\[
  \frac{m_S \sigma_{\max}^2}{2\alpha_c} = \frac{5.0 \times 4 \times 10^{-4}}{0.28}
  \approx 0.007
\]
This is two orders of magnitude smaller than typical EISV signal
...
\begin{align*}
  ...
  &= 1 - \tfrac{0.547}{0.25} = -1.19
\end{align*}
The mean across all 75 active agents is $\bar{m}_{\text{sat}} = -1.23$
(\Cref{app:saturation}), with per-agent variation reflecting
differences in operating-point EISV values.
...
\item \textbf{Population heterogeneity:} The 903 agents include
  ...
  from early development with parameters that have since changed. We report
...
thermodynamically. However, the connection is not purely
metaphorical: Landauer's principle \cite{landauer1961} establishes
that information erasure has irreducible physical cost, suggesting
that an information-theoretic governance framework inherently carries
...
Drift injection & $\lambda_{1}$ & .05--.20 & Adaptive (via $\Theta$) \\
...
\begin{align}
  ...
  F_{23} = F_{32} &= -m_I k \label{eq:F23} \\
  F_{24} = F_{42} &= m_I \beta_I C'(V) - m_V\kappa \label{eq:F24} \\
  F_{34} = F_{43} &= -m_S \lambda_2 C'(V) \label{eq:F34} \\
  F_{14} = F_{41} &= m_V \kappa \label{eq:F14}
\end{align}
where $F_{13}$ and $F_{34}$ are negative (aiding contraction).
Note that $F_{24}$ contains the compound coupling
$m_I \beta_I C'(V) - m_V\kappa$ from the $V$-channel's
effect on~$I$ (via coherence) and $I$'s effect on~$V$
(via the imbalance integral).

\subsection*{Step 2: Contraction Verification}

We verify the contraction condition $F + 2\alpha_c M \preceq 0$
directly by computing the eigenvalues of the symmetric matrix
$F + 2\alpha_c M$ at the production equilibrium.

\begin{remark}
A Gershgorin row-sum analysis is insufficient here: the compound
entry $F_{24}$ contains opposing terms whose partial cancellation
reduces the spectral radius below the row-sum bound. Direct
eigenvalue computation captures this cancellation exactly.
\end{remark}
...
We select the metric weights to balance the coupling between channels,
assigning larger weights to channels that receive more cross-coupling:
\begin{equation}
  M = \operatorname{diag}(1.0, \; 2.5, \; 5.0, \; 2.5)
  \label{eq:metric}
\end{equation}

\subsection*{Step 4: Numerical Verification}

With $M = \operatorname{diag}(1.0, 2.5, 5.0, 2.5)$ and
$\alpha_c = 0.14$, the Jacobian $J$ evaluated at the observed
equilibrium ($E^* = 0.77$, $S^* = 0.08$), and
$C'_{\max} = C_{\max} C_1 \operatorname{sech}^2(0)/2 = 0.5$,
the eigenvalues of $F + 2\alpha_c M$ are:
...
\begin{tabular}{@{}lrl@{}}
\toprule
\textbf{Eigenvalue} & \textbf{Value} & \\
\midrule
$\lambda_1$ & $-6.71$ & \checkmark \\
$\lambda_2$ & $-2.02$ & \checkmark \\
$\lambda_3$ & $-0.46$ & \checkmark \\
$\lambda_4$ & $-0.03$ & \checkmark \\
\bottomrule
\end{tabular}
\end{center}

\smallskip
All eigenvalues are strictly negative, confirming
$F + 2\alpha_c M \prec 0$ and thus
$\mu_M(J) \leq -\alpha_c = -0.14$.

\medskip
\textbf{Result:} With $M = \operatorname{diag}(1.0, 2.5, 5.0,
2.5)$, the contraction analysis certifies $\alpha_c = 0.14$,
consistent with the spectral abscissa of~$J$ at
equilibrium ($-0.15$, Remark~\ref{rem:production-params}).
The key result is $\alpha_c > 0$, guaranteeing global exponential
convergence. \qed
