...
Canonical module for agent business logic, extracted from mcp_server_std.py.
This module owns:
- AgentMetadata class and in-memory agent registry
- Monitor instances and lifecycle
- Metadata loading/saving (PostgreSQL backend)
- Authentication and authorization
- Loop detection and circuit breaking
- Agent archival and lifecycle management
- Standardized agent info building

Transport-specific code (STDIO proxy, MCP protocol handlers) lives in
mcp_server_std.py. HTTP transport lives in mcp_server.py.
"""
...
try:
    ...
    # Minimal stub for environments where MCP SDK isn't installed
    class TextContent:
...
# Set on import; servers can override via init_server_process()
