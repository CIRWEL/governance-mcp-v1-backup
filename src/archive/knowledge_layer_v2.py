...
This module is deprecated and archived. See docs/archive/KNOWLEDGE_LAYER_EXPERIMENT.md

Reason: Redundant with metrics/metadata systems, performance issues, unproven system-wide value.

Replacement: Use markdown files, agent metadata notes, or metrics export instead.

DO NOT USE IN NEW CODE. Preserved for reference only.
...
class KnowledgeManager:
    ...
    def find_related_discoveries(
        ...
        """
        Find related discoveries based on tags, files, or similar summaries.
        Returns list of discovery timestamps that are related.
        """
        related_timestamps = []
        
        if not tags and not related_files and not summary:
            return related_timestamps
        
        # Load all knowledge to search across agents
        ...
        for knowledge_file in knowledge_dir.glob("*.json"):
            try:
                with open(knowledge_file, 'r') as f:
                    data = json.load(f)
                    agent_knowledge = AgentKnowledge.from_dict(data)
                    
                    for disc in agent_knowledge.discoveries:
                        # Skip self
                        if exclude_timestamp and disc.timestamp == exclude_timestamp:
                            continue
                        
                        # Match by tags (any overlap)
                        if tags and disc.tags:
                            if any(tag in disc.tags for tag in tags):
                                related_timestamps.append(disc.timestamp)
                                continue
                        
                        # Match by related_files (any overlap)
                        if related_files and disc.related_files:
                            if any(f in disc.related_files for f in related_files):
                                related_timestamps.append(disc.timestamp)
                                continue
                        
                        # Match by similar summary (simple keyword overlap)
                        if summary and disc.summary:
                            summary_words = set(summary.lower().split())
                            disc_words = set(disc.summary.lower().split())
                            # If 2+ significant words overlap, consider related
                            common_words = summary_words & disc_words
        ...
        return list(set(related_timestamps))
    ...
    def log_discovery(
        ...
        Args:
            auto_link_related: If True, automatically find and link related discoveries.
                              WARNING: This scans all knowledge files and can be slow (blocks Claude Desktop).
        ...
        # Find related discoveries before creating this one (opt-in for performance)
        related = []
        if auto_link_related:
        ...
        discovery = Discovery(
            ...
            tags=tags or [],
            related_files=related_files or [],
            related_discoveries=related[:5]  # Limit to top 5 related
        )
    ...
    def query_discoveries(
        ...
        limit: Optional[int] = 100  # Limit results to prevent context compression
    ) -> List[Discovery]:
        """
        ...
        Args:
            ...
            limit: Maximum number of results to return (default: 100). Prevents context compression.
                   Set to None for unlimited results (not recommended for cross-agent searches).
        """
        results = []
        ...
        for aid in agent_ids:
            ...
            for discovery in knowledge.discoveries:
                ...
                # Early exit if limit reached (before sorting for performance)
    ...
    def find_similar_discoveries(
        ...
        threshold: float = 0.7,
        agent_id: Optional[str] = None,
        limit: Optional[int] = 200  # Limit to prevent context compression
        ...
        Args:
            ...
            limit: Maximum discoveries to check (default: 200). Prevents context compression.
        ...
        all_discoveries = self.query_discoveries(agent_id=agent_id, limit=limit)
...
def log_discovery(agent_id: str, discovery_type: str, summary: str, **kwargs) -> Discovery:
    ...
    Performance note: Set auto_link_related=True to automatically link related discoveries,
    but be aware this scans all knowledge files and can cause unresponsiveness on Claude Desktop.
    """
...
def query_discoveries(**filters) -> List[Discovery]:
    ...
    Performance note: Default limit is 100 results to prevent context compression.
    For cross-agent searches without filters, consider specifying agent_id or using
    more specific filters to reduce the result set.
    """
...
def find_similar_discoveries(summary: str, threshold: float = 0.7, agent_id: Optional[str] = None, limit: Optional[int] = 200) -> List[Tuple[Discovery, float]]:
    """
    Convenience function to find similar discoveries.
    
    Performance note: Default limit is 200 discoveries to prevent context compression.
    """
    return get_knowledge_manager().find_similar_discoveries(summary, threshold, agent_id, limit)
