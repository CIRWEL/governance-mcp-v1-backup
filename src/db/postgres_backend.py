...
class PostgresBackend(DatabaseBackend):
    ...
    # ─── Agent Baselines (ethical drift persistence) ────────────────────
        ...
        try:
            async with self.acquire() as conn:
                ...
                if row is None:
                    return None
                d = dict(row)
                # Convert list column (asyncpg returns list already for TEXT[])
                d.setdefault('recent_decisions', [])
                # Remove updated_at (not part of AgentBaseline dataclass)
