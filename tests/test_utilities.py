...
# ==================== HoldoutValidation Tests ====================

class TestHoldoutValidation:
    """Tests for src/holdout_validation.py"""

    def test_holdout_validator_init_no_file(self):
        """Test HoldoutValidator initialization without existing config"""
        from src.holdout_validation import HoldoutValidator

        with tempfile.TemporaryDirectory() as tmpdir:
            config_path = Path(tmpdir) / "holdout_config.json"
            validator = HoldoutValidator(config_file=config_path)

            assert validator.holdout_agents == set()
            assert validator.enabled == False

    def test_holdout_validator_init_with_file(self):
        """Test HoldoutValidator initialization with existing config"""
        from src.holdout_validation import HoldoutValidator
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert validator.holdout_agents == {"agent1", "agent2"}
            assert validator.enabled == True

    def test_is_holdout_agent(self):
        """Test checking if agent is in holdout set"""
        from src.holdout_validation import HoldoutValidator
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Not enabled, not holdout
            validator.holdout_agents = {"agent1"}
            validator.enabled = False
            assert validator.is_holdout_agent("agent1") == False

            # Enabled, is holdout
            validator.enabled = True
            assert validator.is_holdout_agent("agent1") == True

            # Enabled, not holdout
            assert validator.is_holdout_agent("agent2") == False

    def test_add_remove_holdout_agent(self):
        """Test adding and removing agents from holdout set"""
        from src.holdout_validation import HoldoutValidator
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Add agent
            validator.add_holdout_agent("agent1")
            assert "agent1" in validator.holdout_agents

            # Remove agent
            validator.remove_holdout_agent("agent1")
            assert "agent1" not in validator.holdout_agents

            # Remove non-existent (should not error)
            validator.remove_holdout_agent("nonexistent")

    def test_enable_disable(self):
        """Test enabling and disabling holdout validation"""
        from src.holdout_validation import HoldoutValidator
    ...
    def test_get_holdout_stats(self):
        """Test getting holdout statistics"""
        from src.holdout_validation import HoldoutValidator
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            validator.add_holdout_agent("agent1")
            validator.add_holdout_agent("agent2")
            validator.enable()

            stats = validator.get_holdout_stats()

            assert stats["enabled"] == True
            assert stats["holdout_agents_count"] == 2
            assert set(stats["holdout_agents"]) == {"agent1", "agent2"}
    ...
    def test_config_persistence(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            validator1 = HoldoutValidator(config_file=config_path)
            validator1.add_holdout_agent("agent1")
            validator1.enable()

            # Create second instance from same file
            validator2 = HoldoutValidator(config_file=config_path)

            assert "agent1" in validator2.holdout_agents
            assert validator2.enabled == True
