...
class TestDialecticOperations:

    async def _create_agent(self, backend, agent_id=None):
        """Helper: ensure agent exists for FK constraints."""
        aid = agent_id or _uuid()
        await backend.upsert_agent(aid, "key")
        return aid

    @pytest.mark.asyncio
    async def test_create_dialectic_session(self, backend):
        paused = await self._create_agent(backend)
        session_id = _uuid()
        result = await backend.create_dialectic_session(
    ...
    @pytest.mark.asyncio
    async def test_create_dialectic_session_duplicate(self, backend):
        paused = await self._create_agent(backend)
        session_id = _uuid()
        await backend.create_dialectic_session(session_id=session_id, paused_agent_id=paused)
        result = await backend.create_dialectic_session(session_id=session_id, paused_agent_id=paused)
        assert result["created"] is False

    @pytest.mark.asyncio
    async def test_get_dialectic_session(self, backend):
        paused = await self._create_agent(backend)
        reviewer = await self._create_agent(backend)
        session_id = _uuid()
        await backend.create_dialectic_session(
            session_id=session_id,
            paused_agent_id=paused,
        ...
        session = await backend.get_dialectic_session(session_id)
        assert session is not None
        assert session["session_id"] == session_id
        assert session["paused_agent_id"] == paused
        assert session["reviewer_agent_id"] == reviewer
        assert session["phase"] == "thesis"  # backward-compat alias for status
        assert session["reason"] == "test reason"
        assert session["topic"] == "test topic"

    @pytest.mark.asyncio
    async def test_get_dialectic_session_not_found(self, backend):
        result = await backend.get_dialectic_session("nonexistent")
        assert result is None

    @pytest.mark.asyncio
    async def test_add_dialectic_message(self, backend):
        paused = await self._create_agent(backend)
        session_id = _uuid()
        await backend.create_dialectic_session(session_id=session_id, paused_agent_id=paused)

        msg_id = await backend.add_dialectic_message(
            session_id=session_id,
            agent_id=paused,
            message_type="thesis",
            root_cause="agent stuck on low coherence",
            proposed_conditions=["resume if coherence > 0.5"],
            reasoning="coherence is recovering",
        )
        ...
        assert msg["message_type"] == "thesis"
        assert msg["root_cause"] == "agent stuck on low coherence"
        assert "resume if coherence > 0.5" in msg["proposed_conditions"]

    @pytest.mark.asyncio
    async def test_resolve_dialectic_session(self, backend):
        paused = await self._create_agent(backend)
        session_id = _uuid()
        await backend.create_dialectic_session(session_id=session_id, paused_agent_id=paused)

        result = await backend.resolve_dialectic_session(
            session_id=session_id,
            resolution={"outcome": "resumed", "conditions": ["coherence > 0.5"]},
            status="resolved",
        )
        assert result is True
    ...
    @pytest.mark.asyncio
    async def test_is_agent_in_active_dialectic_session(self, backend):
        paused = await self._create_agent(backend)
        session_id = _uuid()
        await backend.create_dialectic_session(session_id=session_id, paused_agent_id=paused)

        # thesis is an active status
        assert await backend.is_agent_in_active_dialectic_session(paused) is True

    @pytest.mark.asyncio
    async def test_is_agent_in_active_session_after_resolve(self, backend):
        paused = await self._create_agent(backend)
        session_id = _uuid()
        await backend.create_dialectic_session(session_id=session_id, paused_agent_id=paused)
        await backend.resolve_dialectic_session(session_id, {"outcome": "done"})

        assert await backend.is_agent_in_active_dialectic_session(paused) is False

    @pytest.mark.asyncio
    async def test_get_dialectic_session_by_agent(self, backend):
        paused = await self._create_agent(backend)
        session_id = _uuid()
        await backend.create_dialectic_session(session_id=session_id, paused_agent_id=paused)

        session = await backend.get_dialectic_session_by_agent(paused)
        assert session is not None
        assert session["session_id"] == session_id

    @pytest.mark.asyncio
    async def test_get_dialectic_session_by_agent_active_only(self, backend):
        paused = await self._create_agent(backend)
        s1 = _uuid()
        await backend.create_dialectic_session(session_id=s1, paused_agent_id=paused)
        await backend.resolve_dialectic_session(s1, {"outcome": "done"})

        # No active sessions
        result = await backend.get_dialectic_session_by_agent(paused, active_only=True)
        assert result is None

        # Any sessions
        result = await backend.get_dialectic_session_by_agent(paused, active_only=False)
        assert result is not None

    @pytest.mark.asyncio
    async def test_get_all_active_dialectic_sessions_for_agent(self, backend):
        paused = await self._create_agent(backend)
        s1, s2 = _uuid(), _uuid()
        await backend.create_dialectic_session(session_id=s1, paused_agent_id=paused)
        await backend.create_dialectic_session(session_id=s2, paused_agent_id=paused)

        sessions = await backend.get_all_active_dialectic_sessions_for_agent(paused)
        assert len(sessions) == 2

    @pytest.mark.asyncio
    async def test_get_pending_dialectic_sessions(self, backend):
        paused = await self._create_agent(backend)
        s1 = _uuid()
        # Create session without reviewer
        await backend.create_dialectic_session(session_id=s1, paused_agent_id=paused, reason="needs review")

        pending = await backend.get_pending_dialectic_sessions()
        assert len(pending) >= 1
        found = [s for s in pending if s["session_id"] == s1]
        assert len(found) == 1
        assert found[0]["paused_agent_id"] == paused

    @pytest.mark.asyncio
    async def test_dialectic_session_with_messages_and_resolution(self, backend):
        """Full lifecycle: create → thesis → antithesis → synthesis → resolve."""
        paused = await self._create_agent(backend)
        reviewer = await self._create_agent(backend)
        session_id = _uuid()
        ...
        await backend.create_dialectic_session(
            ...
            paused_agent_id=paused,
            reviewer_agent_id=reviewer,
            reason="stuck on coherence drop",
        )

        # Thesis
        await backend.add_dialectic_message(
            session_id, paused, "thesis",
            root_cause="coherence dropped below 0.4",
            proposed_conditions=["wait for coherence > 0.5"],
            reasoning="temporary dip from complex task",
        )

        # Antithesis
        await backend.add_dialectic_message(
            session_id, reviewer, "antithesis",
            concerns=["coherence has been low for 10 minutes"],
            reasoning="pattern suggests deeper issue",
        )

        # Synthesis
        await backend.add_dialectic_message(
            session_id, paused, "synthesis",
            proposed_conditions=["resume with reduced complexity"],
            reasoning="compromise: resume but at lower complexity",
            agrees=True,
        )

        # Resolve
        await backend.resolve_dialectic_session(
            session_id,
            {"outcome": "resumed", "conditions": ["reduced complexity"]},
        )
